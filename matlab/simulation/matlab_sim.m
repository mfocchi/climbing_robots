...
global delta_duration friction_coefficient Fun  Fut Fr l_0  time   VALIDATION DEBUG
g = 9.81;
...
if VALIDATION
    ...
    load ('../optimization/test_matlab_1.mat')
end
...
global   Fun time delta_duration VALIDATION
    if ~VALIDATION
        if (t <= delta_duration)
            fun = Fun;
        else
...
global   Fut time delta_duration VALIDATION
    if ~VALIDATION
        if (t <= delta_duration)
            fut = Fut;
        else
